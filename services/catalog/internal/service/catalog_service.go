...
func (m *CatalogManager) UpdateEvent(ctx context.Context, req *pb.UpdateEventRequest) (*pb.UpdateEventResponse, error) {
	m.logger.Info("UpdateEvent attempt", "title", req.Title, "description", req.Description)
	eventDate, err := time.Parse(time.RFC3339, req.EventDate)
	if err != nil {
	...
	if uuidID, err = uuid.Parse(req.EventId); err != nil {
	...
	updatedEvent, err := m.repo.UpdateEvent(ctx, event)
	if err != nil {
		if errors.Is(err, sql.ErrNoRows) {
		...
		m.logger.Error("UpdateEvent failed: database error", "title", req.Title, "description", req.Description, "error", err)
		return nil, status.Errorf(codes.Internal, "internal server error")
	}
